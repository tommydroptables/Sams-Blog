...
// ------------------------------------------------------------------
//   A lot of the page is based on the size of the screen so
//   every time the page size is change everything needs to be
//   re-alaigned.
// ------------------------------------------------------------------
function allign_things() {
	/* jQuery toggle layout */
	onWindowResize();

    // card hover needs to be re-caluculated every time page is resized
    if(mobileAndTabletcheck())
        set_up_cards_sizes_mobile();
    else
        set_up_cards_sizes();
}
